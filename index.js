...
const upload = multer({
    storage: storage,
    limits: {fileSize: 2000000}, //Gioi han dung luong file la 2MB
...
app.post("/save", upload.single('image'), (req, res) => {
   //Middleware upload.single("image") dam bao chi dinh rang field co name la "image" duoc upload
    ...
    try{
        ...
        const tenMonHoc = req.body.tenMonHoc;
        const hocKy = req.body.hocKy;
        const khoaNganh = req.body.khoaNganh;
        const image = req.file?.originalname.split(".");
        const fileType = image[image.length-1];
        const filePath = `${maMonHoc}_${Date.now().toString()}.${fileType}`
    ...
    try{
        function onDeleteItem(length) { //Dinh nghia de quy xoa
            const params = {
                TableName: tableName,
                Key: {
                    "maMonHoc": listCheckBoxSelected[length]
                }
            }
